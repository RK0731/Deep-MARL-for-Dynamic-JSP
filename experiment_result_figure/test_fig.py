...
panel_A = ['(a.1) Normalized cumulative tardiness\nUtilization rate = 70%','(b.1) Normalized cumulative tardiness\nUtilization rate = 80%','(c.1) Normalized cumulative tardiness\nUtilization rate = 90%']
panel_B = ['(a.2) Win rate %\nUtilization rate = 70%','(b.2) Win rate %\nUtilization rate = 80%','(c.2) Win rate %\nUtilization rate = 90%']
...
for ax_idx in range(3):
    ...
    if ax_idx == 1:
        ...
        # common legend
        name.insert(0,'I-DQN*')
        legend_elements = [Patch(facecolor=pie_color_list[i],label=name[i]) for i in range(benchmark_no)]
    ...
    '''
    anchor = win_rate[-1]
    for idx,w in enumerate(win_rate):
        plt.setp(wedges[idx], radius = w/anchor)
    '''
